"""测试 print-data API 返回结构并使用新的解析器解析"""
import json
import asyncio
import logging
...
async def test():
    async with async_session_factory() as db:
        ...
        # Step 3: 获取课程信息（供 lesson_map 使用）
        lesson_map = {}
        if sem_id and student_id:
            jresp = await client.get(
                f"{COURSE_TABLE_URL}/getLesson",
                params={"semesterId": sem_id, "studentId": student_id},
            )
        ...
        # Step 4: 调用 print-data API
        print_data_url = PRINT_DATA_URL.replace("{sem_id}", str(sem_id))
        print(f"\nCalling print-data API: {print_data_url}")
        print_resp = await client.get(print_data_url, follow_redirects=True)
        print(f"Print-data response: {print_resp.status_code} LEN={len(print_resp.text)}")

        if print_resp.status_code == 200 and len(print_resp.text) > 20:
            print_data = print_resp.json()
            print(f"Print-data keys: {list(print_data.keys()) if isinstance(print_data, dict) else 'N/A'}")

            # 查看 studentTableVms 结构
            vms = print_data.get("studentTableVms", [])
            print(f"studentTableVms count: {len(vms)}")
            for i, vm in enumerate(vms):
                acts = vm.get("activities", [])
                print(f"  VM[{i}] id={vm.get('id')}, name={vm.get('name')}, activities={len(acts)}")
                if acts:
                    # 显示前3个 activity 的结构
                    for j, act in enumerate(acts[:3]):
                        print(f"    Act[{j}]: courseName={act.get('courseName')}, "
                              f"weekday={act.get('weekday')}, "
                              f"startUnit={act.get('startUnit')}, "
                              f"endUnit={act.get('endUnit')}, "
                              f"weekIndexes={act.get('weekIndexes')[:5]}..., "
                              f"room={act.get('room')}, "
                              f"teachers={act.get('teachers')}")

            # 使用新的解析器解析
            schedules = parse_print_data_schedules(print_data, lesson_map)
            print(f"\nParsed {len(schedules)} schedule entries:")
            for s in schedules[:5]:
                print(f"  {s['title']} | 周{s['weekday']} | 第{s['start_unit']}-{s['end_unit']}节 | "
                      f"周次={s['weeks']} | {s['location']} | 教师={s['teacher']}")
            if len(schedules) > 5:
                print(f"  ... and {len(schedules) - 5} more")
        else:
            print(f"Print-data API returned non-200: body={print_resp.text[:500]}")
