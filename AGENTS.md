...
2. **严格遵守流程** — 先详细分析，给出切实可行的细化方案，再进行执行。完成任务后要详细告知用户此次更新的细节。**部署由用户运行 `deploy.bat` 完成**，AI 不操作 Docker 和 HTTP 服务器
...
### AI 代理行为准则
1. **只改代码，不部署** — 修改代码后由用户运行 `deploy.bat` 部署验证
2. **不操作 Docker** — 不启动/停止容器
3. **不启动 HTTP 服务器** — 用户自己运行
4. **凭据自己读** — `.env.local` 自己 read_file，不需要等用户给
5. **只做编译验证** — `flutter build` 验证代码能通过编译即可
