"""Verify ALL events"""
import urllib.request, json
from collections import Counter

tok = open('token.txt').read().strip()

# Query events - use POST to get all
req = urllib.request.Request('http://localhost:8000/api/v1/events/?limit=500',
    headers={'Authorization':f'Bearer {tok}'})
data = json.loads(urllib.request.urlopen(req).read())

fd = [e for e in data if e.get('source')=='fudan']
print(f'Total events: {len(data)}')
print(f'Fudan events: {len(fd)}')

m = Counter()
for e in fd:
    m.update([e.get('event_type','?')])
print(f'By type: {dict(m)}')

m2 = Counter()
for e in fd:
    if e.get('start_time'): m2.update([e['start_time'][:7]])
print(f'Months: {dict(sorted(m2.items()))}')

for e in fd:
    s = e.get('start_time','N/A')[:16] if e.get('start_time') else 'NO TIME'
    et = e['event_type']; 
    rr = (e.get('rrule') or '-')[:20]
    desc = (e.get('description') or '-')[:50]
    print(f'  {e["title"][:25]:25s} | {s:16s} | {et:5s} | {rr:20s} | {desc}')